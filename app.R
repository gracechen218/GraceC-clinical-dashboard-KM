...
ui <- fluidPage(
  navbarPage("User Interface:",
             ...
             tabPanel("Data",
                                        ...
                                        sidebarLayout(
                                          sidebarPanel(
                                            fileInput("file1", "Choose Dataset/File",  #for file upload
                                                      multiple = TRUE,
                                                      accept = c("text/csv",
             ...
             tabPanel(
               ...
               tabsetPanel(
                 tabPanel(
                   ...
                   pageWithSidebar(
                     headerPanel(''),
                     sidebarPanel( selectInput("paramcd_filter", "Select PARAMCD:",  #for KM plot
...
server <- function(input, output, session) {
  ...
  data <- reactive({
    req(input$file1)
     ...
     if(stringr::str_ends(input$file1$datapath, "csv")) {                        #for file upload
  ...
  output$variable_kanl <- renderUI({
    selectInput("variableNames_kanl", label = "Analysis Variable", choices = names(data()))  #renderUI for dynamic populates dropdown menus based on the uploaded data
  })
  ...
  output$plot4 <- renderPlotly({           #creates an interactive Kaplan-Meier plot using `survfit` and `ggplot`
    req(data)
  ...
  output$km_estimate_table <- DT::renderDataTable({   #`renderDataTable` displays the survival object or summary.
    req(survdat())
